...
from odoo.addons.account.tests.test_account_journal_dashboard import TestAccountJournalDashboard


# @freeze_time("2019-01-22")
def test_customer_invoice_dashboard_mod(self):
    ...
    self.assertIn('68.30', dashboard_data['sum_draft'])
    ...
    self.assertIn('81.60', dashboard_data['sum_waiting'])
    ...
    self.assertIn('68.30', dashboard_data['sum_waiting'])
    ...
    self.assertEqual(dashboard_data['number_waiting'], 2)
    self.assertIn('78.30', dashboard_data['sum_waiting'])

    dashboard_data = journal.get_journal_dashboard_datas()
    self.assertEqual(dashboard_data['number_late'], 2)
    self.assertIn('78.30', dashboard_data['sum_late'])


TestAccountJournalDashboard.test_customer_invoice_dashboard = test_customer_invoice_dashboard_mod
